...
class AIOrganizer:
    ...
    def _process_new_event(self, memory_data: Dict[str, Any], event_index: int):
        ...
        try:
            ...
            if event_id in self.completely_processed_entries:
                return  # Already completely processed, skip
            ...
            if self._should_skip_rewriting(event):
    ...
    def _analyze_emotional_tone(self, text: str) -> Dict[str, Any]:
        ...
        neutral_indicators = ['try', 'need', 'want', 'must', 'should', 'often', 'sometimes']
    ...
    def _enhance_event_in_place(self, event: Dict[str, Any], conv_item: Optional[Dict[str, Any]], memory_data: Dict[str, Any], source_info: Optional[Dict[str, Any]] = None):
        ...
        try:
            ...
            # Enhance emotional context with emotion tags
            ...
            # Also consider the provenance context if available
            provenance = event.get('provenance', {})
            if isinstance(provenance, dict) and 'source_info' in provenance:
            ...
            # Analyze emotion tags using the more specific context
            emotion_tags = self._analyze_emotion_tags(context_for_analysis, category_for_tags)
            
            # Update emotional context with new emotion tags
            emotional_context['emotion_tags'] = emotion_tags
            
            # Determine sentiment based on preference indicators in context
            if 'sentiment' not in emotional_context:
                if any(word in context_for_analysis.lower() for word in ['love', 'enjoy', 'like', 'favorite', 'great', 'amazing', 'wonderful', 'fantastic']):
                    emotional_context['sentiment'] = 'positive'
                elif any(word in context_for_analysis.lower() for word in ['hate', 'dislike', 'avoid', 'terrible', 'awful', 'bad']):
                    emotional_context['sentiment'] = 'negative'
                else:
                    emotional_context['sentiment'] = 'positive' if emotion_tags else 'neutral'  # Default to positive for preferences, neutral if no tags
            
            # Determine emotional intensity based on content
            if 'emotional_intensity' not in emotional_context:
                intensity = 0.5  # Default
                if any(word in context_for_analysis.lower() for word in ['love', 'really', 'very', 'absolutely', 'totally', 'extremely', 'always']):
                    intensity = 0.8
                elif any(word in context_for_analysis.lower() for word in ['sometimes', 'occasionally', 'maybe', 'occasional']):
                    intensity = 0.3
                elif any(word in context_for_analysis.lower() for word in ['usually', 'often', 'frequently']):
                    intensity = 0.6
                elif any(word in context_for_analysis.lower() for word in ['never', 'rarely', 'seldom']):
                    intensity = 0.2
                emotional_context['emotional_intensity'] = intensity
            
            # Determine mood context based on content type
            if 'mood_context' not in emotional_context:
                mood = 'normal'
                if any(word in context_for_analysis.lower() for word in ['happy', 'excited', 'energetic', 'motivated']):
                    mood = 'motivated'
                elif any(word in context_for_analysis.lower() for word in ['love', 'enjoy', 'like']):
                    mood = 'happy'
                elif any(word in context_for_analysis.lower() for word in ['health', 'exercise', 'fitness', 'walk']):
                    mood = 'motivated' 
                elif any(word in context_for_analysis.lower() for word in ['coffee', 'tea', 'morning']):
                    mood = 'content'
                elif any(word in context_for_analysis.lower() for word in ['read', 'reading', 'books', 'novels']):
                    mood = 'curious'
                emotional_context['mood_context'] = mood
            
            # Set confidence level
            if 'confidence' not in emotional_context:
                emotional_context['confidence'] = 0.85
...
Enhanced memory entry:"""
    ...
    def _analyze_emotion_tags(self, text: str, category: str) -> List[str]:
        ...
        # Emotion tags based on content keywords
        content_keywords = {
            'food': ['food', 'meal', 'eat', 'dine', 'cuisine', 'pasta', 'pizza', 'italian', 'chinese', 'mexican', 'japanese', 'korean', 'thai', 'indian', 'cooking', 'recipe', 'delicious'],
            'drink': ['drink', 'coffee', 'tea', 'water', 'juice', 'soda', 'wine', 'beer', 'cocktail', 'morning'],
            'health': ['health', 'fitness', 'wellness', 'exercise', 'workout', 'gym', 'yoga', 'meditation', 'sleep', 'running', 'swimming', 'cycling', 'walking'],
            'habit': ['habit', 'routine', 'daily', 'always', 'usually', 'often', 'seldom', 'never', 'schedule', 'timing', 'timing'],
            'reading': ['reading', 'literature', 'books', 'book', 'novel', 'story', 'sci-fi', 'science fiction', 'fantasy', 'novels', 'authors', 'genres', 'magazine', 'article'],
            ...
            'learning': ['learning', 'education', 'study', 'learn', 'school', 'university', 'course', 'class', 'student', 'knowledge'],
