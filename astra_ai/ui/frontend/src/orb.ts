...
export type OrbMood = "neutral" | "good" | "warning" | "error";
// Mood colors:
// neutral: blue (default), good: green, warning: yellow, error: red
const MOOD_COLORS = {
  neutral: 0x4ca8e8,
  good: 0x3ec46d,
  warning: 0xffe066,
...
export function createOrb(canvas: HTMLCanvasElement): Orb {
  ...
  // Speaking-specific (vortex, shockwave, breathing)
  ...
  function animate() {
    ...
    let targetColor = neutralColor;
    if (mood === "good") targetColor = MOOD_COLORS.good;
    else if (mood === "warning") targetColor = MOOD_COLORS.warning;
    else if (mood === "error") targetColor = MOOD_COLORS.error;
    mat.color.lerp(new THREE.Color(targetColor), 0.08);
    lineMat.color.lerp(new THREE.Color(targetColor), 0.08);
  ...
  return {
    ...
    setMood(m: OrbMood) { mood = m; },
    setThemeColor(color: string) {
      try {
        neutralColor = new THREE.Color(color).getHex();
      } catch {
        console.warn(`[ORB] Invalid theme color: ${color}`);
      }
    },
