...
def start_scripts(voice_mode=True):
    """Start all scripts and return their process IDs."""
    ...
    # Start memory_manager.py in a new process
    memory_path = os.path.join(project_root, "memory", "memory_manager.py")
    memory_process = subprocess.Popen([python_executable, memory_path],
                                     creationflags=subprocess.CREATE_NEW_CONSOLE)
    processes["memory"] = {
        "pid": memory_process.pid,
        "name": "memory_manager.py",
        "path": memory_path,
        "last_restart": datetime.now().isoformat()
    }
    print(f"Started Memory Manager with process ID: {memory_process.pid}")
        ...
        while True:
            ...
            for key, process_info in list(processes.items()):
                ...
                if not is_process_running(pid):
                    ...
                    if key == "memory":
                        memory_path = process_info.get("path")
                        memory_process = subprocess.Popen([python_executable, memory_path],
                                                        creationflags=subprocess.CREATE_NEW_CONSOLE)
                        processes["memory"]["pid"] = memory_process.pid
                        processes["memory"]["last_restart"] = datetime.now().isoformat()
                        print(f"Restarted Memory Manager with new process ID: {memory_process.pid}")
